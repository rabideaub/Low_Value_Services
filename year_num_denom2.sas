...
libname out "&out.";
%let windsorize = median; /*Can take on the value outliers, which windorizes at the 5th and 95th percentile, or median, which sets all costs to the median cost*/
...
proc sort data=out.low_value_procs2; by patid /*Fst_dt*/year; run;

data out.low_value_beneficiary_none;
	set out.low_value_procs2 (where=(18<=age<65));
	if ~(1<=age<=100) then age=.;
	*if ~(1<=mdc<=25) then mdc=.;
...
proc means data=out.low_value_procs2;
	var std_cost;
...
proc means data=out.low_value_beneficiary_none;
...
data out.low_value_beneficiary_none;
	merge out.low_value_beneficiary_none (drop=cst_homoc cst_coag cst_pth	
...
data cst_tot (keep=Procedure LV_Group year2011 year2012 year2013 sorter);
	set out.low_value_procs2 (where=(18<=age<65) keep=age year elig_2011 elig_2012 elig_2013 std_cost) end=last;
...
proc sort data=out.low_value_beneficiary_none; by patid; run;
...
%macro winsorize(var);
	%if "&windsorize."="outliers" %then %do;
		...
		proc univariate data=out.low_value_beneficiary_none;
			var cst_&var.;
		...
		data out.low_value_beneficiary_none;
			set out.low_value_beneficiary_none;
...
*%winsorize(eeg_head);
*%winsorize(back);
*%winsorize(carotid_scan_asymp);
*%winsorize(carotid_scan_sync);
*%winsorize(cardio_stress);
*%winsorize(coronary);
*%winsorize(renal_angio);
*%winsorize(carotid_end);
*%winsorize(ivc);
*%winsorize(vertebro);
*%winsorize(arthro);
*%winsorize(t3_test);
...
*%winsorize(pth_s);
*%winsorize(chest_x_s);
*%winsorize(cardio_x_s);
*%winsorize(pft_s); 
*%winsorize(stress_s);
*%winsorize(ct_sinus_s);
*%winsorize(scan_sync_s);
*%winsorize(scan_head_s);
*%winsorize(eeg_head_s);
*%winsorize(back_s);
*%winsorize(carotid_scan_asymp_s);
*%winsorize(carotid_scan_sync_s);
*%winsorize(cardio_stress_s);
*%winsorize(coronary_s);
*%winsorize(renal_angio_s);
*%winsorize(carotid_end_s);
*%winsorize(ivc_s);
*%winsorize(vertebro_s);
*%winsorize(arthro_s);
*%winsorize(t3_test_s);
*%winsorize(spine_inj_s);
*%winsorize(pf_image_s);
*%winsorize(OH_vitamin_s);
**%winsorize(vit_d_s);
**%winsorize(pap_s);
*%winsorize(cyst_s);
*%winsorize(hpv_s);
